...
| **3** | **零干扰（Zero Distraction）** | 无渐变、无模糊阴影、无装饰性圆角、无动画炫技。界面应消失在用户的注意力之外——所见即所需 |
...
| 默认模式 | 浅色 | 浅色 | 浅色 | **深色** |
| 信息密度 | 低（大量留白） | 中 | 中 | **高** |
| 圆角 | 大（16-28px） | 大（8-12px） | 中 | **2-4px** |
| 阴影 | 柔和扩散 | 柔和扩散 | 无 | **层叠阴影（渐进模糊）** |
...
基于 MD3 叠层模型，背景分为 **6 个层级**。层次之间色差控制在 4-7 个亮度单位（L\*），形成**细粒度、逐渐递进**的深度感。在深色模式下，层级越高（越靠近用户），背景越亮。

| Token | Elevation | 色值 | L\* 值 | ΔL | 白色叠加 | 用途 |
|-------|-----------|------|--------|-----|---------|------|
| `--surface-0` | 0dp | `#0A0A0A` | 2.6 | — | 0% | 根背景，桌面/窗口底层 |
| `--surface-1` | 1dp | `#0E0E0E` | 3.6 | +1.0 | 2% | 侧边栏、面板背景、工具栏 |
| `--surface-2` | 3dp | `#131313` | 5.2 | +1.6 | 5% | 卡片、列表组、标签页容器 |
| `--surface-3` | 6dp | `#181818` | 6.9 | +1.7 | 8% | 悬浮卡片、弹出菜单、下拉列表 |
| `--surface-4` | 12dp | `#1E1E1E` | 9.0 | +2.1 | 12% | 对话框、模态窗口、抽屉 |
| `--surface-5` | 24dp | `#252525` | 11.5 | +2.5 | 16% | 工具提示、通知 Toast、最高层级浮层 |

> **L\*** = CIE L\*a\*b\* 亮度值。ΔL（相邻层级亮度差）保持在 1.0-2.5 之间，确保每层都可被感知但不突兀。

### 2.2 阴影系统（Elevation Shadows）

NOTHING UI 使用**多层层叠硬阴影**，低层级（0-3dp）阴影锐利，高层级（6-24dp）引入微小模糊（≤8px）以营造空间感。阴影不柔和不扩散，而是通过**增加层数和偏移量**来增强深度。

| Token | Elevation | 阴影值 | 说明 |
|-------|-----------|--------|------|
| `--shadow-0` | 0dp | `none` | 无阴影，与背景齐平 |
| `--shadow-1` | 1dp | `0 1px 2px rgba(0,0,0,0.40)` | 单层锐利，侧边栏/面板 |
| `--shadow-2` | 3dp | `0 1px 2px rgba(0,0,0,0.40)`<br>`0 2px 4px rgba(0,0,0,0.20)` | 双层，卡片 |
| `--shadow-3` | 6dp | `0 2px 4px rgba(0,0,0,0.45)`<br>`0 4px 8px rgba(0,0,0,0.20)` | 双层加深，悬浮菜单 |
| `--shadow-4` | 12dp | `0 2px 4px rgba(0,0,0,0.50)`<br>`0 8px 16px rgba(0,0,0,0.25)` | 引入 16px 模糊，对话框 |
| `--shadow-5` | 24dp | `0 4px 8px rgba(0,0,0,0.55)`<br>`0 12px 24px rgba(0,0,0,0.30)` | 最深阴影，工具提示 |
...
| 状态 | 主色 | 背景色（半透明） | 用途 |
|------|------|-----------------|------|
| **Success** | `#4AA26F` | `rgba(74,162,111,0.12)` | 操作成功、状态正常 |
...
NOTHING UI 使用**极微圆角**（2px），既保留了工业感，又消除纯直角在低分辨率屏幕上的锯齿感。

| Token | 值 | 用途 |
|-------|-----|------|
| `--radius-none` | 0px | 面板边缘、表格、代码块（内部元素） |
| `--radius-sm` | 2px | **默认**：按钮、卡片、搜索栏、输入框 |
| `--radius-md` | 4px | 对话框、弹出菜单、工具提示 |
| `--radius-full` | 9999px | 进度条端点、标签徽标、spinner |

> **原则**：能用 2px 的地方绝不用更大的值。4px 仅用于需要视觉柔软度的最高层浮层。
...
:root {
  ...
  /* ---- Elevation Shadows (层叠硬阴影) ---- */
  --shadow-0: none;
  --shadow-1: 0 1px 2px rgba(0, 0, 0, 0.40);
  --shadow-2: 0 1px 2px rgba(0, 0, 0, 0.40),
              0 2px 4px rgba(0, 0, 0, 0.20);
  --shadow-3: 0 2px 4px rgba(0, 0, 0, 0.45),
              0 4px 8px rgba(0, 0, 0, 0.20);
  --shadow-4: 0 2px 4px rgba(0, 0, 0, 0.50),
              0 8px 16px rgba(0, 0, 0, 0.25);
  --shadow-5: 0 4px 8px rgba(0, 0, 0, 0.55),
  ...
  /* ---- Status ---- */
  --color-success:       #4AA26F;
  --color-success-bg:    rgba(74, 162, 111, 0.12);
...
NOTHING UI 提供三种按钮样式（填充、轮廓、文本）+ 一种危险按钮变体，均使用 2px 极微圆角（`border-radius: var(--radius-sm)`）。
...
卡片用于分组相关内容。NOTHING UI 的卡片极其克制：2px 极微圆角、层叠渐变阴影、使用 6 层 surface 叠层区分。
...
.card {
  ...
  border-radius: var(--radius-sm);
  padding: 16px;
  box-shadow: var(--shadow-2);       /* 双层阴影：近层锐利 + 远层柔和 */
}
```

#### 卡片层级

卡片层级映射到 6 层 Surface 系统，每层有不同的阴影和背景：

| 层级 | Surface | 阴影 | border-radius | 用途 |
|------|---------|------|---------------|------|
| Flat | `--surface-2` | `var(--shadow-0)` | 2px | 嵌入面板中的内联卡片 |
| Slightly Raised | `--surface-2` | `var(--shadow-2)` | 2px | 可交互卡片、设置卡片 |
| Elevated | `--surface-3` | `var(--shadow-3)` | 2px | 悬浮卡片、下拉面板、弹出菜单 |
| Modal | `--surface-4` | `var(--shadow-4)` | 4px | 对话框、模态窗口 |
| Top-Level | `--surface-5` | `var(--shadow-5)` | 4px | 工具提示、通知 Toast |
...
::-webkit-scrollbar-thumb {
  ...
  border-radius: var(--radius-sm);  /* 2px 极微圆角 */
}
...
[data-theme="light"] {
  ...
  --color-success:       #3B8C5A;
  --color-success-bg:    rgba(74, 162, 111, 0.10);
...
```json
{
  "nothing-ui": {
    ...
    "colors": {
      "surface": {
        "0": { "dark": "#0A0A0A", "light": "#FAFAFA", "elevation": 0 },
        "1": { "dark": "#0E0E0E", "light": "#F2F2F2", "elevation": 1 },
        "2": { "dark": "#131313", "light": "#EBEBEB", "elevation": 3 },
        "3": { "dark": "#181818", "light": "#E5E5E5", "elevation": 6 },
        "4": { "dark": "#1E1E1E", "light": "#E0E0E0", "elevation": 12 },
        "5": { "dark": "#252525", "light": "#D6D6D6", "elevation": 24 }
      },
      "shadow": {
        "0": "none",
        "1": "0 1px 2px rgba(0,0,0,0.40)",
        "2": "0 1px 2px rgba(0,0,0,0.40), 0 2px 4px rgba(0,0,0,0.20)",
        "3": "0 2px 4px rgba(0,0,0,0.45), 0 4px 8px rgba(0,0,0,0.20)",
        "4": "0 2px 4px rgba(0,0,0,0.50), 0 8px 16px rgba(0,0,0,0.25)",
        "5": "0 4px 8px rgba(0,0,0,0.55), 0 12px 24px rgba(0,0,0,0.30)"
      },
      "text": {
      ...
      "accent": {
        ...
        "green":  { "default": "#4AA26F", "hover": "#5BB87D", "active": "#3B8C5A", "bg": "rgba(74,162,111,0.12)" },
        "orange": { "default": "#FF9800", "hover": "#FFA726", "active": "#F57C00", "bg": "rgba(255,152,0,0.12)" }
      },
      "status": {
        "success": { "color": "#4AA26F", "bg": "rgba(74,162,111,0.12)" },
...
3. **覆盖 MD3 默认值**：圆角 → 2px（极微）、阴影 → 层叠渐变阴影、Surface → 6 层细粒度
...
ThemeData createNothingTheme({Brightness brightness = Brightness.dark}) {
  ...
  return ThemeData(
    ...
    colorScheme: ColorScheme(
      ...
      surface: isDark ? const Color(0xFF0E0E0E) : const Color(0xFFF2F2F2),
      surfaceContainer: isDark ? const Color(0xFF131313) : const Color(0xFFEBEBEB),
      surfaceContainerHigh: isDark ? const Color(0xFF1E1E1E) : const Color(0xFFE0E0E0),
      surfaceContainerHighest: isDark ? const Color(0xFF252525) : const Color(0xFFD6D6D6),
    ...
    filledButtonTheme: FilledButtonThemeData(
      style: FilledButton.styleFrom(
        ...
        shape: const RoundedRectangleBorder(borderRadius: BorderRadius.circular(2)),
...
| # | 必需项 | 具体要求 | 验证方式 |
|---|--------|---------|---------|
| **1** | **颜色系统** | Surface 0-5 六个层级 + 1 套强调色 + 3 级文本色 + 5 级阴影 | 视觉检查：深色默认，6 层 Surface 分明，阴影递进 |
...
| **4** | **核心组件样式** | 按钮（填充/轮廓/文本/危险）、列表项（32px + 2px 左侧指示器）、卡片（2px 圆角 + 层叠阴影）、进度条（4px）、搜索栏（32px）、标签页（底部指示器） | 每个组件覆盖全部状态（默认/悬停/选中/禁用） |
...
| `--surface-1` | `#0E0E0E` | `#F2F2F2` |
| `--surface-2` | `#131313` | `#EBEBEB` |
| `--surface-3` | `#181818` | `#E5E5E5` |
| `--surface-4` | `#1E1E1E` | `#E0E0E0` |
| `--surface-5` | `#252525` | `#D6D6D6` |
| `--text-primary` | `#F5F5F5` | `#1A1A1A` |
| `--text-secondary` | `#B0B0B0` | `#616161` |
| `--text-tertiary` | `#707070` | `#9E9E9E` |
| `--accent` (Cyan) | `#00BCD4` | `#00838F` |
| `--accent` (Green) | `#4AA26F` | `#3B8C5A` |
| `--accent` (Orange) | `#FF9800` | `#E65100` |
| `--color-success` | `#4AA26F` | `#3B8C5A` |
...
| Token | Elevation | Value |
|-------|-----------|-------|
| `--shadow-0` | 0dp | `none` |
| `--shadow-1` | 1dp | `0 1px 2px rgba(0,0,0,0.40)` |
| `--shadow-2` | 3dp | `0 1px 2px rgba(0,0,0,0.40), 0 2px 4px rgba(0,0,0,0.20)` |
| `--shadow-3` | 6dp | `0 2px 4px rgba(0,0,0,0.45), 0 4px 8px rgba(0,0,0,0.20)` |
| `--shadow-4` | 12dp | `0 2px 4px rgba(0,0,0,0.50), 0 8px 16px rgba(0,0,0,0.25)` |
| `--shadow-5` | 24dp | `0 4px 8px rgba(0,0,0,0.55), 0 12px 24px rgba(0,0,0,0.30)` |

### 形状

| Token | Value | 用途 |
|-------|-------|------|
| `--radius-none` | 0px | 内部元素：面板、表格、代码块 |
| `--radius-sm` | 2px | 默认：按钮、卡片、输入框、搜索栏 |
| `--radius-md` | 4px | 浮层：对话框、弹出菜单、工具提示 |
| `--radius-full` | 9999px | 圆形：进度条端点、标签徽标 |
...
| `--ease-emphasized` | `cubic-bezier(0.05, 0.0, 0.0, 1.0)` |
